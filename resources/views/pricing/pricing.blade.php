            ...
            <div class="container mx-auto px-4 md:px-6">
                <div class="text-center max-w-5xl mx-auto">
                    ...
                    <h1 class="text-5xl md:text-7xl font-extrabold text-gray-900 mb-8 tracking-tight">
                        Choose Your Perfect
                        <span class="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent"> Plan</span>
                    </h1>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-10 max-w-7xl mx-auto">
                    @foreach([
                            [
                                ...
                                'icon' => 'camera',
                                'features' => [
                                    'Basic facial recognition for photo tagging (limited to 5 searches/day)',
                                    'Photo grouping by date and time only',
                                    'Basic search & retrieval with keywords and metadata filters (no advanced filters)',
                                    'Watermarked photo previews',
                                    'Limited secure cloud storage (up to 1GB)',
                                    'Offline mode for pre-downloaded content',
                                    'Basic social sharing (with watermarks)',
                                    'Access to e-commerce for purchases (no subscriptions or premium merch discounts)'
                                ],
                                'recommended' => false,
                                'cta' => 'Sign Up Free'
                            ],
                            [
                                'name' => 'Basic Plan',
                                'description' => 'Ideal for photographers and enthusiasts',
                                'price' => 4.99,
                                'gradient' => 'from-purple-500 to-pink-500',
                                'icon' => 'star',
                                'features' => [
                                    'All Free Plan features',
                                    'Unlimited facial recognition searches and real-time face-matching',
                                    'Full photo grouping (by Tour Provider, Location, Date, Time, and events)',
                                    'Advanced search filters (date range, photographer, location)',
                                    'Basic photo enhancement & editing (cropping, filters, brightness)',
                                    'Increased secure cloud storage (up to 10GB) with private albums',
                                    'Social sharing without mandatory watermarks and customizable templates',
                                    'In-app/website purchases with integration to payment gateways',
                                    'Instant notifications for new photos and offers'
                                ],
                                'recommended' => true,
                            ...
                            [
                                ...
                                'features' => [
                                    ...
                                    'AI-powered photo enhancements (sharpening, color correction, background removal)',
                                    'AI stickers, overlays, and full editing tools',
                                    'Unlimited secure cloud storage with end-to-end encryption and custom access controls',
                                    'Photo collaboration for shared albums',
                                    'Location-based services (GPS tagging)',
                                    'Subscription perks: Unlimited access to premium features, priority support, and 20% discount on personalized merchandise (calendars, mugs, T-shirts, etc.)',
                                    'Analytics for personal photo trends',
                                    'Offline access to all content, plus AI experiences on mobile'
                                ],
                                'recommended' => false,
                                'cta' => 'Start Free Trial'
                            ]
                            ...
                            >
                                @if($plan['recommended'])
                                    <div class="absolute top-0 left-0 right-0 bg-gradient-to-r from-indigo-500 to-violet-500 text-white text-center py-3 text-base font-medium">
                                        {{ $plan['badge'] }}
                                    </div>
                                @endif

                                <div class="text-center @if($plan['recommended']) pt-16 @else pt-12 @endif pb-8">
                                    <div class="w-16 h-16 bg-gradient-to-r {{ $plan['gradient'] }} rounded-2xl flex items-center justify-center mx-auto mb-6 text-white shadow-lg">
                                        @if($plan['icon'] == 'camera')
                                    ...
                                    <p class="text-gray-600 mt-2 text-lg">{{ $plan['description'] }}</p>

                                    <div class="mt-8">
                                        <div class="flex items-baseline justify-center">
                                            <span class="text-6xl font-extrabold text-gray-900">$</span>
                                            <span class="text-6xl font-extrabold text-gray-900">{{ $plan['price'] }}</span>
                                ...
                                <div class="px-10 pb-10">
                                    <div class="space-y-4 mb-10 mt-8">
                                        @foreach($plan['features'] as $feature)
                                            <div class="flex items-start space-x-3">
                                                <svg class="w-6 h-6 text-emerald-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    ...
                                    <button 
                                        class="w-full py-4 text-lg font-semibold rounded-full transition-all duration-300 shadow-lg"
                                        @if($plan['recommended']) bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white @else bg-gray-900 hover:bg-gray-800 text-white shadow-md @endif
                                    >
                                        {{ $plan['cta'] }}
        ...
        <section class="py-24 bg-gradient-to-br from-gray-50 to-indigo-50">
            <div class="container mx-auto px-4 md:px-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10 max-w-6xl mx-auto">
                    @foreach([
                            ['name' => 'Extra Storage', 'desc' => 'Additional cloud storage space', 'price' => '$5/month per 100GB', 'icon' => 'cloud'],
                            ['name' => 'Premium Support', 'desc' => '24/7 priority support with phone access', 'price' => '$15/month', 'icon' => 'phone'],
                            ['name' => 'Advanced Analytics', 'desc' => 'Detailed insights and reporting', 'price' => '$10/month', 'icon' => 'trending-up'],
                            ['name' => 'Custom Branding', 'desc' => 'White-label solution with your branding', 'price' => '$25/month', 'icon' => 'award'],
                        ] as $addon)
                            <div class="text-center bg-white border-0 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden rounded-3xl transform hover:-translate-y-2">
                                <div class="p-8 md:p-10">
                                    <div class="w-14 h-14 bg-gradient-to-r from-indigo-500 to-violet-500 rounded-2xl flex items-center justify-center mx-auto mb-6 text-white shadow-lg">
                                        @if($addon['icon'] == 'cloud')
                                        ...
                                        @elseif($addon['icon'] == 'award')
                                            <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2H9m0-2h6a2 2 0 012 2v12a2 2 0 01-2 2H9a2 2 0 01-2-2V3a2 2 0 012-2z"></path></svg>
                                        @endif
                                    </div>
            ...
            <div class="container mx-auto px-4 md:px-6">
                ...
                <div class="max-w-5xl mx-auto">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                        @foreach([
                                ['question' => 'Can I change my plan anytime?', 'answer' => 'Yes, you can upgrade or downgrade your plan at any time. Changes take effect immediately, and we\'ll prorate the billing accordingly.'],
                                ['question' => 'Is there a free trial?', 'answer' => 'Yes, we offer a 14-day free trial for all plans. No credit card required to start your trial.'],
                                ['question' => 'What happens to my photos if I cancel?', 'answer' => 'You can download all your photos before canceling. We provide a 30-day grace period to export your data.'],
                                ['question' => 'Do you offer refunds?', 'answer' => 'Yes, we offer a 30-day money-back guarantee for all paid plans. Contact support for assistance.'],
                                ['question' => 'Is my data secure?', 'answer' => 'Absolutely. We use enterprise-grade encryption and security measures to protect your photos and personal data.'],
                                ['question' => 'Can I use Blue Star Memory for commercial purposes?', 'answer' => 'Yes, our Basic and Premium plans include commercial usage rights. Check our terms for specific details.'],
                            ] as $faq)
                                <div class="bg-white border-0 shadow-xl hover:shadow-2xl transition-all duration-300 rounded-3xl overflow-hidden">
                                    <div class="p-8 md:p-10">
        ...
        <section class="py-24 bg-gradient-to-br from-gray-50 to-indigo-50">
            <div class="container mx-auto px-4 md:px-6">
                <div class="max-w-5xl mx-auto text-center">
                    <div class="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-3xl p-12 md:p-16 text-white shadow-2xl">
                        <svg class="w-20 h-20 mx-auto mb-8 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                        ...
                        <div class="flex flex-col sm:flex-row gap-6 justify-center">
                            <button class="inline-flex items-center bg-white text-indigo-600 font-semibold px-10 py-4 rounded-full hover:bg-gray-100 transition-all duration-300 shadow-lg hover:shadow-xl text-lg">
                            ...
                            <button class="inline-flex items-center border-2 border-white text-white font-semibold px-10 py-4 rounded-full hover:bg-white/10 transition-all duration-300 shadow-lg hover:shadow-xl text-lg">
            ...
            <div class="container mx-auto px-4 md:px-6 text-center">
                <div class="max-w-5xl mx-auto">
                    ...
                    <p class="text-xl md:text-2xl text-indigo-100 mb-12 max-w-3xl mx-auto">
                        Join thousands of users who have transformed their photo management experience with Blue Star Memory.
                    </p>
                    <a href="/get-started" class="inline-flex items-center bg-white text-indigo-600 font-semibold px-12 py-5 rounded-full hover:bg-gray-100 transition-all duration-300 shadow-lg hover:shadow-xl text-lg">
                        ...
                        <svg class="w-6 h-6 ml-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </a>
                </div>
            </div>
        </section>
